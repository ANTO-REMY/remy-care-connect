...
BEGIN
  -- Ensure healthworker exists
  SELECT type INTO chw_type FROM healthworkers WHERE id = NEW.chw_id;

  IF chw_type IS NULL THEN
    RAISE EXCEPTION 'CHW with id % does not exist', NEW.chw_id;
  END IF;

  -- Ensure it is actually a CHW, not a nurse
  IF chw_type <> 'chw' THEN
    RAISE EXCEPTION 'Assigned healthworker % is not a CHW', NEW.chw_id;
  END IF;
